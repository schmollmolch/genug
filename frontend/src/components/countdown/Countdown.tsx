...
const getCurrentSecondsLeft = (props: Props): number => {
  return props.timer.status === "running"
    ? moment(props.timer.started)
        .add(props.timer.remainingSecondsSinceLastStart, "seconds")
        .diff(moment().local(), "seconds")
    ...
    <IonCard>
      ...
      <IonCardContent>
        <CountdownRunningState
          state={props.timer.status}
          pause={props.pauseTimer}
          continue={props.continueTimer}
        ></CountdownRunningState>
      </IonCardContent>
    </IonCard>
  );
